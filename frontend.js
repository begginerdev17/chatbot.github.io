...
function onTimerFinish() {
  if (!chatState.chatIn) {
    ...
    if (now - chatState.lastClickTime >= 1000) {
      ...
      // Si se quiere mostrar mensaje de "Continue" en la interfaz del chat
      //appendMessage(
        // "user",
        // "Continue"
        //);
...
async function clearMemory() {
  try {
    ...
    if (result.cleared) {
      console.log(" Historial y memoria de conversación borrado");
      messages.splice(0, messages.length);
      renderMessages();
      setStatus("Historial y memoria de conversación borrado.");
      chatState.chatIn = false;
      chatState.canSendContinue = true;
      updateSendButtonState();
      updateContinueButtonState();
